...
- Extend `db/init.sql` with `agent.incidents` (status, fingerprint, service, resource, failure_class, first_seen, last_seen) and a small `agent.ingest_cursor` (single row, `last_seen_time timestamptz`) — replaces the originally-planned `processed` boolean, which turned out not to be viable on `pf_logs_raw` (see Phase 4 §3: the table can only ever have exactly the 3 columns Fluent Bit's positional insert requires).
- `agent/detector.py` — two responsibilities, run as one poll loop:
  1. **Normalize pass**: `SELECT tag, time, data FROM public.pf_logs_raw WHERE time > :last_seen_time ORDER BY time` in small batches (time high-water-mark against `agent.ingest_cursor`, not a per-row `processed` flag — `pf_logs_raw` has no `id`/flag column to hold one). Parse `tag` (e.g. `pf.server.admin`) into `log_type`/`pf_role`; redact secret-shaped values from `data`; extract `severity`/`logger`/`thread`/`message`/`exception_type`/`exception_message`/`stack_trace`/`tracking_id`/`namespace`/`pod_name` directly from `data`'s consistently-named JSON fields (direct field access, not fuzzy parsing — confirmed field names against real rows observed in Phase 4, not just the template); normalize IDs/timestamps/paths out of the message; compute `error_signature`; insert the normalized row into `agent.pf_logs`; advance `agent.ingest_cursor.last_seen_time` transactionally with the batch insert (same transaction, so a crash mid-batch never advances the cursor past rows that weren't actually written).
...
**Risk:** the time high-water-mark approach has a real edge case worth testing deliberately — two rows with the exact same `time` value (millisecond collisions are plausible at PF's log volume) straddling a batch boundary could cause one to be skipped. Confirm the batch query's boundary condition (`>` vs `>=` plus a small overlap-and-dedupe-by-content window) before trusting this at Sim A/B scale, not just at the row counts seen in Phase 4's smoke test.
