...
- **Simulation count: staying at two** (Sim A, Sim B `ImagePullBackOff`), not expanded to the README's seven lettered scenarios. Sim A was originally the `SERVER_PROFILE_PATH` profile-path trap; live-tested and redefined to `POSTGRES_JDBC_URL` corruption instead (see Phase 5's "Live test findings, and Sim A redefined") once the profile-path trap turned out to produce zero log signal. The README's other scenarios (JDBC placeholder, OOM, profile-not-applied, protected-credential escalation) remain good V2 candidates but out of scope for this plan.
...
### Live test findings, and Sim A redefined

Both faults below were triggered for real against the cluster (corrupted a value in `helm/ping-devops/values.yaml`, ran `deploy-helm.sh`, observed the result, then reverted and redeployed to restore health — nothing was left broken) before deciding anything.

**`SERVER_PROFILE_PATH` trap (the fault this plan originally called "Sim A") — triggered, and the result ruled it out as the V1 Sim A.** Corrupting it to `helm/server-profile/instance` produced **zero log signal of any kind** — not a WARN, not an ERROR, nothing mentioning "merge," "hook," "clone," or "skip" in any log file. Both pods came up `2/2 Running` as `golden-architecture.md` predicted, but the entire git-sourced profile was silently ignored — not just the path itself: `server-json.log` didn't exist (our log4j2.xml patch never merged), and `PostgresPfApp` (our custom JDBC datastore from `data.json.subst`) was completely absent from the running config. This fault is fundamentally undetectable by `detector.py`'s log-watching approach — there is no log line to ever match. It would need a separate reconciler (live env var vs. `agent.config_baseline.golden_value`, no log parsing at all) that doesn't exist yet. **Decision: deferred out of V1 — no longer one of the two locked-in sims, kept documented here as a known, real, distinct failure mode for a future phase, not implemented as a simulation script.**

**`POSTGRES_JDBC_URL` corrupted to a nonexistent host — triggered as an exploratory validation, then promoted to be the new Sim A.** Both pods again came up `2/2 Running` (PF's JDBC datastore check doesn't block readiness), but a real, repeated `ERROR` landed in the logs: `"Unable to load custom data source instance: JDBC-FD67494D8AAFD9D8A5D00C310DE08DD078626978"` — three fresh occurrences. Running `detector.py` against it worked exactly as designed: all three collapsed into the *same* incident as an earlier, unrelated historical occurrence of the identical fault, correctly reaching `occurrence_count = 4` rather than creating duplicates — the first live confirmation `detector.py` dedups a genuinely real fault, not just synthetic test rows. **Decision (made directly, not left open): this is now Sim A for V1.** It's log-detectable by the already-built `detector.py` with no new detection component needed, unlike the profile-path trap above.
...
2. ~~Trigger Sim A for real~~ — **done, and it redefined Sim A**: see Phase 5's "Live test findings, and Sim A redefined" above. The original `SERVER_PROFILE_PATH` candidate produced zero log signal and got deferred out of V1; `POSTGRES_JDBC_URL` corruption is the new Sim A, is log-detectable by the already-built `detector.py`, and is implemented as `Error_Simulation/sim_a_jdbc_url.sh` (built, pushed, not yet run). **Next up:** actually run `sim_a_jdbc_url.sh inject`, confirm `agent.incidents` catches it the same way the manual test did, then build the minimal UI on top of that — no reconciler needed to unblock this anymore, that's deferred-scope work now.
...
- Simulation count stays at two throughout — Sim A: `POSTGRES_JDBC_URL` corruption (redefined from the original `SERVER_PROFILE_PATH` candidate, deferred out of V1 for producing no log signal — see Phase 5), implemented as `Error_Simulation/sim_a_jdbc_url.sh`; Sim B: `ImagePullBackOff` via bad `image.tag`, not yet implemented as a script. Not the seven-scenario set from the reconciliation-pass spec, which are good V2 candidates but out of scope here.

## Critical files

- `setup.sh` — the one command for a fresh clone (after `infrastructure/.env` is filled in)
- `Error_Simulation/sim_a_jdbc_url.sh` — Sim A, inject/restore `POSTGRES_JDBC_URL` corruption
